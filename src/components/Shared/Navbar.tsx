...
import { Menu, Facebook, Twitter, Linkedin, Github, Code } from "lucide-react"
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div className="relative">
          <nav className="flex items-center justify-between border border-border rounded-2xl bg-background/90 backdrop-blur-md p-0">
            {/* Menu Trigger - Hidden on mobile, shown on desktop */}
            <div className="hidden md:flex">
              <button className="p-4 text-center w-16 h-16 border-r border-border rounded-l-2xl hover:bg-muted transition-colors">
                <Menu className="w-5 h-5 mx-auto" />
              </button>
            </div>

            {/* Main Content Container */}
            <div className="flex-1 flex flex-row-reverse lg:flex-row items-center justify-between py-2 px-4 gap-4">
              {/* Logo */}
              <Link href="/" className="flex items-center">
                <div className="rounded mr-2">
                  <Code className="w-6 h-6 sm:w-7 sm:h-7" />
                </div>
                <span className="text-lg sm:text-xl font-semibold">Shohaib.dev</span>
              </Link>
              ...
              <div className="flex items-center space-x-3">
                ...
                >
                  <div className="w-5 h-4 flex flex-col justify-between">
                    <span
                      className={`block h-0.5 bg-current transition-transform ${isMobileMenu ? "rotate-45 translate-y-1.5" : ""}`}
                    />
                    <span className={`block h-0.5 bg-current transition-opacity ${isMobileMenu ? "opacity-0" : ""}`} />
                    <span
                      className={`block h-0.5 bg-current transition-transform ${isMobileMenu ? "-rotate-45 -translate-y-1.5" : ""}`}
                    />
                  </div>
